function getLabyrinthes() {
    return data
}

let lab = $("<div></div>").attr("id", "labyrinthe")

function initCases(size, difficulty) {
    let cases = $("<div></div>").attr("class", "cases")
    if (c.entrance) {
...
function initLabyrinte(size, difficulty) {
    for (c of data[size][difficulty]) {
        initCases(size, difficulty)
    }
}

initLabyrinte("8", "ex-0")